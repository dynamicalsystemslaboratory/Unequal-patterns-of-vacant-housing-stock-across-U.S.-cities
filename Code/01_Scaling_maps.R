...
map_fill_legend <- ggplot(data = df_MSA, aes(x = x, y = y, fill = perCapita_vac)) +
  ...
  scale_fill_gradientn(colors = reds, name = "Vacant houses per capita", trans = "log", breaks = custom_breaks) +
  theme_void() +
...
map_population <- ggplot() +
  ...
  geom_point(
    data = df_MSA,
    aes(x = x, y = y, size = POPESTIMATE, fill = perCapita_occup),
    shape = 21, color = "lightgrey", alpha = 0.8
  ) +
  scale_fill_gradientn(
    colors = greens,
    name = "Occupied houses per capita",
    trans = "log",
    na.value = "blue",
    breaks = custom_breaks,
    guide = "none"
...
map_fill_legend <- ggplot(data = df_MSA, aes(x = x, y = y, fill = perCapita_occup)) +
  geom_point(shape = 21, size = 5) +
  scale_fill_gradientn(colors = greens, name = "Occupied houses per capita", trans = "log", breaks = custom_breaks) +
  theme_void() +
  theme_classic(base_size = 8) +
  guides(
    fill = guide_colorbar(
...
p_ACHS_vac_ratio <- ggplot(df_MSA, aes(x = ratio_vac)) +
  ...
  xlab("Percentage of vacant houses") + 
  ylab("Frequency")+
...
p_ACHS_vac_noSeasonal_ratio <- ggplot(df_MSA, aes(x = ratio_vac_noSeasonal)) +
  ...
  xlab("Percentage of nonseasonal vacant houses") + 
  ylab("Frequency") +
...
p_ACHS_vac_other_ratio <- ggplot(df_MSA, aes(x = ratio_vac_other)) +
  ...
  theme(
    ...
    panel.grid = element_blank(),
    text = element_text(family = "Arial")) +
  xlab("Percentage of \"other\" vacant houses") + 
  ylab("Frequency") +
  scale_x_continuous(limits = c(0, 18), breaks = seq(0, 18, by = 1), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 80), breaks = seq(0, 80, by = 10), expand = c(0, 0))
...
ggsave(filename = "Figures/SI_MSA_vacancyRatio_hist2022_alldefs.pdf", plot = p, width = 180, height = 90, units = "mm", dpi = 900, device = cairo_pdf)
...
p_ratio_vac <- ggplot() +
  ...
  geom_sf(
    data = states_shp,fill = NA, color = "black", linewidth = 0.3) +
  scale_fill_gradientn(
    colors = reds,
    name = "Percentage of vacant houses",
    trans = "log",
    na.value = "white",
    breaks = custom_breaks,
...
map_population <- ggplot() +
  ...
  scale_fill_gradientn(colors = reds, trans = "log", breaks = custom_breaks, name = "Vacant houses per capita", na.value = "darkblue") +
  theme_classic(base_size = 10) +
  theme(
    plot.margin = margin(0.1, 0.1, 0.1, 0.1, "cm"),
    axis.title = element_blank(),
...
map_population <- ggplot() +
  ...
  geom_sf(data = states_shp, fill = NA, color = "black", size = 0.3) +
  scale_fill_gradientn(colors = greens, trans = "log", breaks = custom_breaks, name = "Occupied houses per capita", na.value = "darkblue") +
  theme_classic(base_size = 10) +
  theme(
    plot.margin = margin(0.1, 0.1, 0.1, 0.1, "cm"),
    axis.title = element_blank(),
